...
import "package:home_manager/features/water/water_period_list_tile.dart";
...
class WaterPageState extends State<WaterPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final items = await widget.water.list(widget.home.id);
      if (mounted) {
        ...
        AppLog.i("Loaded ${items.length} water periods");
      }
    } catch (e, st) {
      AppLog.e("Failed to load water periods", error: e, stackTrace: st);
      if (mounted) {
        setState(() {
          _error = "$e";
          _loading = false;
  ...
  Future<void> _deletePeriod(WaterPeriod period) async {
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading && _items.isEmpty) {
      return const WaterPageSkeleton();
    }
        ...
        child: ListView(
          ...
          children: [
            ...
            if (_items.isEmpty) ...[
              ...
              AnimatedEntrance(
                index: 1,
                child: WaterSummaryCard(home: widget.home, periods: _items),
              ),
